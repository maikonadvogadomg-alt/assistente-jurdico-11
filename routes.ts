...
async function geminiStreamMessages(
  ...
  const ownGeminiKey2 = getLocalConfig("gemini_api_key");
  let client = gemini;
  if (customKey && !customUrl) {
    client = new GoogleGenAI({ apiKey: customKey });
  } else if (!customKey && ownGeminiKey2) {
    client = new GoogleGenAI({ apiKey: ownGeminiKey2 });
  }

  const geminiKey2 = process.env.AI_INTEGRATIONS_GEMINI_API_KEY;
  const geminiUrl2 = process.env.AI_INTEGRATIONS_GEMINI_BASE_URL;
  if (!customKey && !ownGeminiKey2 && geminiKey2 && geminiUrl2) {
...
export async function registerRoutes(
  ...
  app.get("/download/assistente-juridico.zip", (_req, res) => {
    const zipPath = path.resolve("public/assistente-juridico-COMPLETO.zip");
    if (!fs.existsSync(zipPath)) {
      res.status(404).json({ error: "ZIP não encontrado" });
      return;
    }
    res.setHeader("Content-Disposition", "attachment; filename=assistente-juridico-COMPLETO.zip");
...
7. Se for sobre legislação, cite o artigo de forma natural na frase`;
      ...
      // Sem chave ou chave inválida → Gemini (demo)
  ...
  app.post("/api/jurisprudencia/buscar", async (req, res) => {
    try {
      ...
      const DATAJUD_PUBLIC_FALLBACK = "cDZHYzlZa0JadVREZDJCendQbXY6SkJlTzNjLV9TRENyQk1RdnFKZGRQdw==";
      const rawKey = clientKey?.trim() || process.env.DATAJUD_API_KEY || DATAJUD_PUBLIC_FALLBACK;
      ...
      try {
        ...
        for (const file of files) {
          ...
          try {
            ...
            const isText =
              ["txt", "md", "csv", "json", "js", "ts", "py", "sql", "log"].includes(ext) ||
            ...
            if (isPdf) {
                ...
                try {
                  ...
                  for (const pageFile of pageFiles) {
                    const { stdout } = await execFileAsync(
                      "tesseract",
                      [path.join(ocrTmpDir, pageFile), "stdout", "-l", "por+eng"],
                      { timeout: 60000 },
                    );
                    ocrText += stdout + "\n";
                  }
              ...
              // OCR direto em imagem (JPG, PNG, etc.)
              const ocrTmpDir = fs.mkdtempSync(path.join("/tmp", "ocr-img-"));
              try {
                const imgPath = path.join(ocrTmpDir, `img.${ext || "png"}`);
                fs.writeFileSync(imgPath, file.buffer);
                const { stdout } = await execFileAsync(
                  "tesseract",
                  [imgPath, "stdout", "-l", "por+eng"],
                  { timeout: 90000 },
                );
              ...
              // .doc antigo — tenta extrair texto bruto (sem LibreOffice)
              try {
                const result = await mammoth.extractRawText({ buffer: file.buffer });
                extractedText = result.value;
              } catch {
                extractedText = file.buffer.toString("latin1").replace(/[^\x20-\x7E\xC0-\xFF\n\r\t]/g, " ").replace(/\s{3,}/g, "\n");
              }
            } else if (isHtml) {
              ...
              // Tenta UTF-8 como último recurso
              extractedText = file.buffer.toString("utf-8");
            }
          } catch (err) {
  ...
  app.post("/api/ai/process", async (req, res) => {
    try {
      ...
      if (isCustom) {
        ...
        // ── Gemini/Groq/OpenAI (usa chave salva nas configurações automaticamente) ──
        const dbKey = (await storage.getSetting("demo_api_key")) || "";
        const dbUrl = (await storage.getSetting("demo_api_url")) || "";
        const dbModel = (await storage.getSetting("demo_api_model")) || "";
        ...
        // Verifica chaves salvas nas configurações (Groq, OpenAI, custom4) como fallback automático
        const savedGroqKey = getLocalConfig("groq_api_key") || "";
        const savedOpenAiKey = getLocalConfig("openai_api_key") || "";
        const savedCustom4Key = getLocalConfig("custom4_api_key") || "";
        const savedCustom4Url = getLocalConfig("custom4_api_url") || "";
        const savedCustom4Model = getLocalConfig("custom4_api_model") || "";
        const autoFallbackKey = savedGroqKey || savedCustom4Key || savedOpenAiKey || cleanKey(dbKey);
        const autoFallbackUrl = savedGroqKey ? "https://api.groq.com/openai/v1"
          : savedCustom4Key ? savedCustom4Url
          : savedOpenAiKey ? "https://api.openai.com/v1"
          : dbUrl;
        const autoFallbackModel = savedGroqKey ? "llama-3.3-70b-versatile"
          : savedCustom4Key ? (savedCustom4Model || "gpt-4o-mini")
          : savedOpenAiKey ? (model === "economico" ? "gpt-4o-mini" : "gpt-4o")
          : dbModel;
        const rawOwnKey = cleanKey(((customKey as string) || "").trim() || (process.env.PUBLIC_API_KEY || "").trim() || autoFallbackKey);
        const ownKey = rawOwnKey.length > 10 ? rawOwnKey : "";
        const detected = ownKey ? autoDetectProvider(ownKey) : null;
        const ownUrl = ((customUrl as string) || "").trim() || (process.env.PUBLIC_API_URL || "").trim() || autoFallbackUrl || (detected?.url || "");
        const ownModel = ((customModelName as string) || "").trim() || (process.env.PUBLIC_API_MODEL || "").trim() || autoFallbackModel || (detected?.model || "");
        if (ownKey) {
      ...
      const _usedProvider = isPerplexity ? "Perplexity" : "Gemini/Demo";
...
FORMATACAO OBRIGATORIA: Use paragrafos CURTOS, com no maximo 4 a 5 linhas cada. Sempre pule uma linha em branco entre cada paragrafo. Nao crie blocos de texto longos ou embolados. NAO use asteriscos (*), hashtags (#) ou qualquer sintaxe markdown. Para titulos use CAIXA ALTA. Cada paragrafo em uma unica linha continua, sem quebras no meio da frase.`;
      ...
      if (isCustomRefine) {
        ...
        const savedGroqKeyR = getLocalConfig("groq_api_key") || "";
        const savedOpenAiKeyR = getLocalConfig("openai_api_key") || "";
        const savedC4KeyR = getLocalConfig("custom4_api_key") || "";
        const savedC4UrlR = getLocalConfig("custom4_api_url") || "";
        const savedC4ModelR = getLocalConfig("custom4_api_model") || "";
        const autoFallbackKeyR = savedGroqKeyR || savedC4KeyR || savedOpenAiKeyR || cleanKeyR(dbKeyR);
        const autoFallbackUrlR = savedGroqKeyR ? "https://api.groq.com/openai/v1" : savedC4KeyR ? savedC4UrlR : savedOpenAiKeyR ? "https://api.openai.com/v1" : dbUrlR;
        const autoFallbackModelR = savedGroqKeyR ? "llama-3.3-70b-versatile" : savedC4KeyR ? (savedC4ModelR || "gpt-4o-mini") : savedOpenAiKeyR ? "gpt-4o-mini" : dbModelR;
        const rawOwnKeyR = cleanKeyR(((refineCustomKey as string) || "").trim() || (process.env.PUBLIC_API_KEY || "").trim() || autoFallbackKeyR);
        const ownKeyR = rawOwnKeyR.length > 10 ? rawOwnKeyR : "";
        const detectedR = ownKeyR ? autoDetectProvider(ownKeyR) : null;
        const ownUrlR = ((refineCustomUrl as string) || "").trim() || (process.env.PUBLIC_API_URL || "").trim() || autoFallbackUrlR || (detectedR?.url || "");
        const ownModelR = ((refineCustomModelName as string) || "").trim() || (process.env.PUBLIC_API_MODEL || "").trim() || autoFallbackModelR || (detectedR?.model || "");
        if (ownKeyR) {
  ...
  app.get("/api/settings/ai-config", requireAuth, async (_req, res) => {
    try {
      const keys = ["gemini_api_key", "openai_api_key", "groq_api_key", "perplexity_api_key", "custom4_api_key", "custom4_api_url", "custom4_api_model", "demo_api_key", "demo_api_url", "demo_api_model"];
      const cfg = (await import("./local-config")).readLocalConfig();
      const result: Record<string, string> = {};
      for (const k of keys) {
        result[k] = (cfg as any)[k] || "";
      }
  ...
  app.put("/api/settings/ai-config", requireAuth, async (req, res) => {
    try {
      const allowed = ["gemini_api_key", "openai_api_key", "groq_api_key", "perplexity_api_key", "custom4_api_key", "custom4_api_url", "custom4_api_model", "demo_api_key", "demo_api_url", "demo_api_model"];
      const { setLocalConfig } = await import("./local-config");
      for (const k of allowed) {
        if (typeof req.body[k] === "string") {
          setLocalConfig(k as any, req.body[k].trim());
          if (req.body[k].trim()) process.env[k.toUpperCase()] = req.body[k].trim();
        }
      }
      res.json({ ok: true });
    } catch (e: any) {
  ...
  app.get("/api/settings/system-status", requireAuth, async (_req, res) => {
    try {
      ...
      res.json({
        ...
        hasGroqKey: !!cfg.groq_api_key,
        hasPerplexityKey: !!cfg.perplexity_api_key,
        hasCustom4Key: !!cfg.custom4_api_key,
  ...
  app.get("/api/settings/db-tables", requireAuth, async (_req, res) => {
    try {
      const { db: currentDb } = await import("./storage");
      const result = await currentDb.execute(
        "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' ORDER BY table_name" as any
      );
      const tables = (result.rows || []).map((r: any) => r.table_name);
      res.json({ ok: true, tables });
    } catch (e: any) {
      res.json({ ok: false, tables: [], error: e.message });
    }
  });
  ...
  app.post("/api/settings/test-ai-key", requireAuth, async (req, res) => {
    ...
    try {
      // Groq (gsk_...)
      if (provider === "groq" || effectiveKey.startsWith("gsk_")) {
        const client = new OpenAI({ apiKey: testKey, baseURL: "https://api.groq.com/openai/v1" });
        await client.chat.completions.create({
          model: "llama-3.1-8b-instant",
          messages: [{ role: "user", content: "responda apenas: ok" }],
          max_tokens: 5,
        });
        return res.json({ ok: true, message: "Chave Groq funcionando!" });
      }

      // OpenAI (sk-...)
      if (provider === "openai" || (effectiveKey.startsWith("sk-") && !effectiveKey.startsWith("sk-ant-") && !effectiveKey.startsWith("sk-or-"))) {
        const client = new OpenAI({ apiKey: testKey, baseURL: "https://api.openai.com/v1" });
        await client.chat.completions.create({
          model: "gpt-4o-mini",
          messages: [{ role: "user", content: "responda apenas: ok" }],
          max_tokens: 5,
      ...
      // 4ª Chave Personalizada (custom) — testa com URL configurada
      if (provider === "custom") {
        const customUrl = getLocalConfig("custom4_api_url") || "https://openrouter.ai/api/v1";
        const customModel = getLocalConfig("custom4_api_model") || "gpt-4o-mini";
        const client = new OpenAI({ apiKey: testKey, baseURL: customUrl });
        await client.chat.completions.create({
          model: customModel,
          messages: [{ role: "user", content: "responda apenas: ok" }],
          max_tokens: 5,
        });
        return res.json({ ok: true, message: `Chave personalizada funcionando! (${customUrl})` });
      }
      ...
      const friendly = msg.includes("API_KEY_INVALID") || msg.includes("invalid") || msg.includes("Incorrect API key")
        ...
        : msg.includes("PERMISSION_DENIED") ? "Chave sem permissão — verifique no painel se a API está ativada."
